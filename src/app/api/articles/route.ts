...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!response.ok) {
      ...
      try {
        const errorData = responseText ? JSON.parse(responseText) : { error: 'No error details' };
    ...
    if (!responseText || responseText.trim() === '') {
      console.log('Empty response from Directus (200 OK) - returning empty data structure');
